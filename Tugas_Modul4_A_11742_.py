...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    clusters = model.fit_predict(data[features])  # Melakukan clustering pada data yang ada
    data[f"{model_name}_Cluster"] = clusters

    if model_name == "KMeans_model":
        new_cluster = model.predict(new_point[features])[0]  # Prediksi langsung untuk KMeans
    else:
        distances = pairwise_distances(new_point[features], data[features])  # Menghitung jarak antar titik
        nearest_index = distances.argmin()  # Menemukan index titik terdekat
        new_cluster = clusters[nearest_index]  # Menentukan cluster berdasarkan titik terdekat
...
st.set_page_config(
    page_title="11742 - Unsupervised Learning",  # Ganti dengan NPM Anda
    page_icon="",
    layout="wide",
    initial_sidebar_state="expanded"
)

# File uploader untuk input CSV
uploaded_file = st.sidebar.file_uploader("Upload your input CSV file", type=["csv"])

if uploaded_file is not None:
    input_data = pd.read_csv(uploaded_file)
    st.markdown("<h1 style='text-align: center;'>Unsupervised Learning - Radit</h1>", unsafe_allow_html=True)
    st.dataframe(input_data)

    # Direktori tempat penyimpanan ketiga model yang telah di dump sebelumnya
    model_directory = r'C:\Unsupervised Learning (Praktek)\TUGAS4_A_11742'
    model_path = {
    ...
    if st.sidebar.button("Prediksi !"):
        ...
        # Membuat dan menampilkan plot untuk setiap model
